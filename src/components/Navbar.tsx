    ...
    <nav className="bg-[#3d3b3a]/95 backdrop-blur-md sticky top-0 z-50 border-b border-white/10">
      <div className="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3 group">
          <img
            src="https://res.cloudinary.com/dhkh3ixfd/image/upload/v1779941602/andani/l25if7dmfh9tc1wrwvma.jpg"
            alt="Logo Empresa"
        ...
        {/* BOTÓN HAMBURGUESA */}
        <button className="md:hidden text-[#F2F2F2] focus:outline-none" onClick={() => setOpen(!open)} >
        ...
        <div className="hidden md:flex space-x-8 text-sm font-medium text-[#F2F2F2] items-center">
          {navLinks.map((link) => (
            <Link 
              key={link.path} 
              to={link.path} 
              className="hover:text-amber-400 transition-all duration-250 relative group"
            >
              {link.name}
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-amber-400 transition-all duration-350 group-hover:w-full"></span>
            </Link>
          ))}
